<template>
  <select v-model="place_id" @change="place_change(valye,this.place_id)"
  ...
  <select v-model="index" @change="selectChanged(value,this.index)"
          style="width:200px;height: 30px;margin-top: 5px;margin-bottom: 5px;margin-left: 10px">
    <option v-for="item in kindList" :value="item.name" :label="item.name"></option>
  </select>
  <!-- echarts -->
  <div id="monthdataBox" :style="{width: '300px', height: '300px'}"></div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      myChart: null,
      placeData: [],
      kindList: [{name: "so2", id: 1}, {name: "co", id: 2}, {name: "no2", id: 3}, {
        name: "pm25",
        id: 4
      }, {name: "temperature", id: 5}, {name: "humidity", id: 6}]
    }
  },
  created() {
    this.load();
  ...
  methods: {
    loadData() {
      request.get("/place/getData").then(res => {
    ...
    drawLine() {
      ...
      this.myChart = echarts.init(document.getElementById('monthdataBox'));
      // 绘制图表
      this.myChart.setOption({
        title: {text: '最近一月SO2变化'},
        tooltip: {},
        xAxis: {
          data: ["1", "2", "3", "4"]
        },
    ...
    load() {
      requests.get("/data/month/" + this.place_id).then(res => {
